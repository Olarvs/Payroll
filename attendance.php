	...
	<script type="text/javascript">
		$(document).ready(function(){
			...
			$('.remove_attendance').click(function(){
				...
				_conf("Are you sure to archive this employee's time log record?","remove_attendance",[d])
			})
			$('.rem_att').click(function(){
				var $id=$(this).attr('data-id');
				_conf("Are you sure to archive this time log?","rem_att",[$id])
			})
		});
		function remove_attendance(id){
			...
			$.ajax({
				...
				success:function(resp){
						if(resp == 1){
							alert_toast("Selected employee's time log data successfully archived","success");
								setTimeout(function(){
								location.reload();
